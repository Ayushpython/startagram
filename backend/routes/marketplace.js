...
router.get('/categories', async (req, res) => {
  try {
    ...
    for (const category of categories) {
      categoryStats[category] = await Blueprint.countDocuments({
        category,
        status: 'published'
      });
    }
...
router.get('/recommendations', authMiddleware, async (req, res) => {
  try {
    ...
    userBlueprints.forEach(bp => {
      bp.tags.forEach(tag => userTags.add(tag));
    });
...
// @route   POST /api/marketplace/:blueprintId/rate
// @desc    Rate a blueprint
// @access  Private
router.post('/:blueprintId/rate', authMiddleware, async (req, res) => {
  try {
    ...
    const existingReview = blueprint.reviews.find(r => r.reviewer.toString() === req.user.userId);
    
    if (existingReview) {
      existingReview.rating = rating;
      existingReview.comment = comment || '';
    } else {
      blueprint.reviews.push({
